...
# Poor quality APIs/data
@timefunc
def get_newsdata(stock_tickers):
    ...
    if len(stock_tickers) <= 0:
        print("No stock tickers to process.")
        return
    ...
    for n, q in enumerate(querystrings, 1):
        print(str(n) + ". ", q)
    #print(querystrings)
    #input("Press a key . . .")
    responses = {
        k: requests.request("GET", url, headers=headers, params=v).json()
        for k, v in querystrings.items()
    }
    stock_news = {
        k: response["canvassMessages"] for k, response in responses.items()
    }
    for k, v in stock_news.items():
        print(f"Stock: {k} === {v}{NL}News:{NL}'''{NL}{v}{NL}'''{NL}{NL}")
...
def sentiment_score(stock_ticker):
    ...
    num_comments = len(data_for_comments)
    import os
